    ...
    if st.button("📥 Charger les Feux", type="primary", use_container_width=True):
        with st.spinner("Loading fire data from NASA FIRMS..."):
            if filter_type == "Country" and country_code:
                fires = load_nasa_fires(
                    country=country_code.upper(), 
                    days=days
                )
            elif filter_type == "Bounding Box" and bbox:
                # Pour bbox, on charge quand même les données mondiales pour l'instant
            ...
            if len(fires) > 0:
                ...
                st.warning("⚠️ Aucun feu trouvé. Essayez une autre zone ou période.")
            st.rerun()
...
with col_map:
    ...
    # Carte avec villes (OpenStreetMap - par défaut)
    folium.TileLayer(
